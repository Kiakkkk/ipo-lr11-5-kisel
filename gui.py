...
import json
import os
from transport import Client, TransportCompany, Airplane, Train
class TransportApp:
        ...
        if os.path.exists("clients.json"):
            with open("clients.json", "r") as f:
                ...
                for client_data in clients_data:
                    client = Client.Client(client_data['name'], client_data['cargo_weight'], client_data['is_vip'])
            ...
            with open("transport.json", "r") as f:
                ...
                for vehicle_data in transport_data:
                    if vehicle_data['type'] == 'Train':
                        vehicle = Train.Train(vehicle_data['capacity'], vehicle_data['number_of_cars'])
                    elif vehicle_data['type'] == 'Airplane':
                        vehicle = Airplane.Airplane(vehicle_data['capacity'], vehicle_data['max_altitude'])
    ...
    def add_client(self):
        ...
        if not name or not cargo_weight or cargo_weight <= 0:
            ...
            return
        client = Client.Client(name, cargo_weight, is_vip)
        self.company.add_client(client)
    ...
    def add_vehicle(self):
        vehicle_type = simpledialog.askstring("Тип транспорта", "1 - Train 2 - Airplane:")
        capacity = simpledialog.askfloat("Добавить транспорт", "Введите грузоподъемность транспорта (в тоннах):")
        if vehicle_type == "1":
            number_of_cars = simpledialog.askstring("Поезд", "Количество вагонов: ")
            vehicle = Train.Train(capacity, number_of_cars)
        elif vehicle_type == "2":
            max_altitude = simpledialog.askstring("Добавить транспорт", "Максимальная высота: ")
            vehicle = Airplane.Airplane(capacity, max_altitude)
        else:
    ...
    def edit_vehicle(self, event):
        ...
        vehicle_type = simpledialog.askstring("Редактировать транспорт","1 - Train 2 - Airplane:", initialvalue=type(vehicle).__name__.lower())
        ...
        if vehicle_type == "1":
            number_of_cars = messagebox.askyesno("Поезд", "Новое количество вагонов: ", default=vehicle.number_of_cars)
            vehicle = Train.Train(capacity, number_of_cars)
        elif vehicle_type == "2":
            max_altitude = simpledialog.askstring("Редактировать транспорт", "Максимальная высота: ", initialvalue=vehicle.max_altitude)
            vehicle = Airplane.Airplane(capacity, max_altitude)
        else:
            messagebox.showerror("Ошибка", "Некорректный тип транспорта")
            return
    ...
    def show_about(self):
        messagebox.showinfo("О программе", "ЛР 12\nВариант: 5\nФИО разработчика: Кисель К.В.")
    #удаление
